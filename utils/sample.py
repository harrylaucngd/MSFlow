...
def cond_generate_mols(model, cond, guidance_scale=0.0, num_samples = 1000, steps = 100,path = path, seq_len=MAX_LEN,source_distribution='uniform',mask_token_id=0, device = 'cuda',temperature = temperature):
    # samples = sample_flow_custom(model=model,vocab_size=173,seq_len=128,num_samples=num_samples,source_distribution='masked',mask_token_id=0,device='cuda')
