...
cfm_module = CondFlowMolBERTLitModule.load_from_checkpoint('checkpoints/')
model = cfm_module.model
model.eval()
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model.to(device)
condition_df = pd.read_csv('computed_conditions/msg_cddd_conditions_df.csv') 
query_smiles = condition_df.canon_smiles
cddd_list = condition_df["cddd"].tolist()
...
print('Example generated SMILES:', smiles[0])