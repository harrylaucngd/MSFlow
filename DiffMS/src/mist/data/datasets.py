...
def _collate_pairs(input_batch: list, mol_collate_fn: Callable, spec_collate_fn: Callable) -> dict:
    ...
    # === Inject encoder-required fields ===
    # number of peaks per spectrum
    num_peaks = torch.tensor([len(spec["peak_type"]) for jj in input_batch for spec in jj["spec"]])
    # peak types
    # types = torch.tensor([spec["peak_type"] for jj in input_batch for spec in jj["spec"]])
    all_peak_types = np.vstack([spec["peak_type"] for jj in input_batch for spec in jj["spec"]])
    types = torch.from_numpy(all_peak_types).float()
    # instrument IDs
    instruments = torch.tensor([spec.get("instrument", 0) for jj in input_batch for spec in jj["spec"]])
...
def get_paired_loader_graph(
    ...
    # Pass collate function correctly
    collate_fn = lambda batch: _collate_pairs(batch, mol_collate_fn, spec_collate_fn)
